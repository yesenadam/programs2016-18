...
fn="/Downloads-1/Panjin-Red-Beach-China--620x614.png"#"/Downloads-1/newpicssmall/sumatran-tiger_crop_0.png"
...
#				ave=(a[j][k][i]+a[j+1][k][i]+a[j][k+1][i]+a[j+1][k+1][i])/4
			...
			#try add all its 8 neighbours... less checkerboardy then..
				...
				tot=(a1+a2+a3+a4+b1+b2+b3+b4)/8#4.0
			#	ave=128-(128-ave)*(1+enhance)
			#	if ave>255:
			#		ave=255
			#	if ave<0:
	...
	#b2=np.rot90(b,3)
	#b2=np.fliplr(b2)
	name=na+"-%d+%d-v3-enhanced.png" % (endy,endx) #yes, its on its side 8|
	misc.imsave(name,b)
