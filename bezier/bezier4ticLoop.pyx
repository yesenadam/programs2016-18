...
#bezier4ticLoop.pyx
#does multiple random 4tic beziers..
DEF MAXPTS = 5
DEF WID=1280
DEF HEI = 720
DEF STEPS=500
DEF SCENES=10 #num of different bezier movies


###
###NB!! Plot is wrong i think - need to multiply the +1 at the loop end by the inc (1/-1)
# otherwise it leaves a gap..that explains the gap at the end of lines!!! FIX
###
import numpy as np
cimport numpy as np
...
cpdef OK():
	cdef int i,j,k,col,loops,tally=0, scene
	...
	for scene in xrange(SCENES):
		for loops in xrange(STEPS):
			...
			print scene,tally,loops
			fn="/Users/admin/bezier/bezier-%05d.png" % tally			
			misc.imsave(fn,img)
			tally+=1
		#now do more frames after motion stops
		#maybe 2 seconds
		for i in xrange(60):
			fn="/Users/admin/bezier/bezier-%05d.png" % tally			
			misc.imsave(fn,img)
			tally+=1
		for i in xrange(5):
			P[i]=[rand()%(WID-100)+50,rand()%(HEI-100)+50]
