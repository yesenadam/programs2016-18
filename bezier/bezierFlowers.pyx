...
#draw lots of random points, well-spaced.
#than select 4 at a time to use a bezier control points.
#(then shift points slightly, make movie.)
#have array of points, choose different ones to be P[0]-[4] each time.

DEF MAXPTS = 5
DEF LOTSPTS=16 #number of rand points to have
DEF WID=1280
DEF HEI = 720
DEF STEPS=500
import numpy as np
...
cdef struct rgb:
	int col[3]
cdef pt P[MAXPTS], points[LOTSPTS]
cdef int cimg[WID][HEI][3]
cdef rgb white,green, blue, red, rg, gb,grey
...
cpdef OK():
	...
	for i in xrange(LOTSPTS):
		points[i].x=rand()%(WID-100)+50
		points[i].y=rand()%(HEI-60)+30
	#use all combination of 5 points?
	cdef int which[MAXPTS], w0, w1, w2, w3, w4
	for i in xrange(MAXPTS):
		which[i]=-1
	#but NEII this way, points[0] will always be an end control point, never anything else.
	for w0 in xrange(LOTSPTS-4):
		which[0]=w0
		for w1 in xrange(which[0]+1,LOTSPTS-3):
			which[1]=w1
			for w2 in xrange(which[1]+1,LOTSPTS-2):
				which[2]=w2
				for w3 in xrange(which[2]+1,LOTSPTS-1):
					which[3]=w3
					for w4 in xrange(which[3]+1,LOTSPTS):
						which[4]=w4
						print which[0],which[1],which[2],which[3],which[4]
						for i in xrange(MAXPTS):
							P[i]=points[which[i]]
#							print which[i],P[i]
#						print
						for loops in xrange(STEPS): #get pts in bezier path
...
#	print loops
	fn="/Users/admin/bezierFlowers1.png" #-%05d.png" % loops			
	misc.imsave(fn,img)
	print tally