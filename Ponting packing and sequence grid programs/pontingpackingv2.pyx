#pontingpacking.pyx - v2 - 10 Oct 2017
...
cdef int highest, count=1

cdef SetSquareSize(int a,int b):
	global count
	Square[a][b].size = count
	count += 1
...
cdef GetSizes():
	...
	for i in xrange(N-1,-1,-2): # in C: for (i=N-1;i>-1;i=i-2) {
		SetSquareSize(i,N-1)
	for j in xrange(N-2,0,-2): #rest of rows in pairs
		for i in xrange(N-2,0,-2):
			SetSquareSize(i,j)
		for i in xrange(N-1,-1,-2):
			SetSquareSize(i,j-1)
	for j in xrange(1,N-1,2): 	#do right column
		SetSquareSize(N-1,j)
	for i in xrange(N-2,0,-2): 	#rest of columns in pairs
...
cdef GetPositions():
	...
	for j in xrange(1,N-1,2): 	#do rest of rows in pairs
