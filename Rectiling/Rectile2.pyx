...
#--------------------------------------------------
DEF cx=40*4 #x-pos of central rect in grid - NB keep as a multiple of 4.
...
DEF TILPICSIZE=18 #size of tiling pgn produced
...
DEF MAXDIM=2*cx 
DEF gridwidth=cx-8 # number of cells x cells to show on grid. must be <cx,cy
DEF H=gridwidth/2 #<-- NB must be even!! won't work otherwise (so cx must be 4n)
DEF TILH=H-2#1 #half-width of tiling to draw
cdef:
	bint InfoAdded
	int i,j,k
	struct r: 
