#include <iostream>
//Pack v2 - square packing. uses a square/rectangular grid of squares of size 1..X*Y
//This version uses wss[] (Where is Square of Size [x] ?)
//to store which v[] is using each square size, or none (0). (not used)
//Saves the old Used() function having to look through every v[] for that info.

//***NB!! am 1/2way through adding the wss bit, doesnt work currently....

using std::cout;
void FindHColumn(unsigned char col);
const unsigned char X=5, Y=5; // rows and cols in packing i.e. X across by Y down
const unsigned char tot=X*Y;
unsigned char v[tot+1], wss[tot+1], c=0;
...
for (v[1]=1;v[1]<tot+1;v[1]++) {
	for (int i=2;i<tot+1;i++) wss[i]=0; //when prog comes back here, strip back to v[1] only.
	wss[v[1]]=1; //ie square of size v[1] is used by v number 1. v[1].
	FillFirstColumn(2);
