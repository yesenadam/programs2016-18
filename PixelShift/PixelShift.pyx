...
#GRR!! I cant get the c-ndarrays to work.
#Seems they must be inside the main function - but can't be in functions inside the main function... arghh.
#so moving things around doesnt help... transfer the little functions
...
b=np.empty([yy,xx,3],dtype=int)#np.uint8)#DTYPE_t) # i THINK these need to be reversed..
...
# returns orig coord (w=0->x,w=1->y) of current point x,y.
	return h[x][y][w]
cdef int Offset(int x,y,w): #gives dist from home, w=0 x coord, =1 y coord
...
#swapping more pixels..	print "Pixel Shift v1"
...
#check how far the pixels move. may work, may not - i.e. improve image
	...
	b2=np.rot90(b,3)
	b2=np.fliplr(b2)
	misc.imsave('point-reyes-398+342-7331-pixshiftv1.png',b2)
