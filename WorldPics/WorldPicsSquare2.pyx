...
cpdef OK():
	...
	pic=misc.imread("/Downloads-1/picsforcython/010 copy.png") #coast.png")/Users/admin/Desktop/aerialsydney.png")#picsforcython/gonzalez.png")#
...
#	cdef int currdiff=0, delta=100 #starting val .. 100?
	for i in xrange(xx):
		for j in xrange(yy):
			for k in xrange(3):
				p[i,j,k]=0 #black
	cdef int side, smallest=3 #smallest square side allowed
	...
	for k in xrange(LOOPS):
		...
		dd=(ttotal-ptotal)/(side*side) #ie if pos, pic needs more of colour
