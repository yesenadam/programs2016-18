...
from polygonedgecolor import polygon2d #this is in admin/sageupdates folder
...
DEF close=130 #counted as close colour. maybe make more sensitive!!
cdef int pjk[3], xx, yy, close, c
...
name=na+"-%d" % close+"-rows-t.png" #saves under this name, e.g. room-180.png ##NB put "rows" in name if it does rows first..
a=misc.imread(fn)
...
#check if new top rh square is already used.. maybe another square already there
					if used[j][k+side]==0: # 'rows' version. swap back
#top right sq of new sq size already used in a square, so use current size
						break
					else:
						for q in xrange(side+2):
							if not Similar(j+q,k+side): #not the same col
								found=1
								break 
						if not found: #this line to avoid deccing side twice
							for q in xrange(side+1):
								if not Similar(j+side,k+q):
									found=1
									break
					if not found:
						side+=1
