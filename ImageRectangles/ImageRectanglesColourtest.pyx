#ImageRectanglesColourtest.pyx - Sept 2016
...
DEF close=20 #counted as close colour.
DEF maxSideRatio=3 #no rectangles allowed with side ratio more than this.
#puts pixels with all R,G,B closer than 30 in the same square. 80=bigger squares, less sensitive. 
cdef int pjk[3], xx, yy, close
fn="/Downloads-1/newpicssmall/sunflowers-wallpaper-1280x720.png"#/Downloads-1/newpicssmall/sumatran-tiger_crop_0.png"#
...
cdef inline bint Similar(int xp, yp): #=1/true if all 3 cols in p[xp][yp] are similar to cols of p[j][k]
	...
	for c in range(3): #ive no idea why using p[j][k][c] here doesnt work! pjk[c] seems the same to me.
		tot+=abs(p[xp][yp][c]-pjk[c])
	if tot>2*close: #not a match
		return 0
	return 1
				...
				if found==1: #blocked in x-dir so add rows in y-direction if possible
					while found<3:
						if side2==maxSideRatio*side or j+side>xx-2 or k+side2>yy-2:
							...
							side2+=1					
						if used[j][k+side2-1]==0: #maybe another rect already there
							found=3 #i.e. y side blocked
							side2-=1
							break
						for q in xrange(side): #check added row in ydirection
							if not Similar(j+q,k+side2-1):
				...
				if found==2: #blcked in y-dir so add columns in x-direction
					while found<3:
						if side==maxSideRatio*side2 or j+side>xx-2 or k+side2>yy-2:
							...
							side+=1					
						if used[j+side-1][k]==0: #maybe another rect already there
							found=3 #i.e. y side blocked
							side-=1
							break
...
#		print xx-j
	...
	name=na+"-close%d" % close+"-maxsr%d" % maxSideRatio+"-%drects-colourtest.png" % tally
